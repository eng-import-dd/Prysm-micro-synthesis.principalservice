        ...
        private async Task ReportHealth(ServiceContext context, CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        var info = new HealthInformation(context.NodeContext.NodeName, "Principal-healthCheck", HealthState.Error)
                        {
                    ...
                    {
                        var info = new HealthInformation(context.NodeContext.NodeName, "Principal-healthCheck", HealthState.Ok);
                        Partition.ReportInstanceHealth(info);
                    }
                }
